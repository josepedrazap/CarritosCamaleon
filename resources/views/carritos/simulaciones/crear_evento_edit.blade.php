...
<body>
  ...
  <input name="id_evento" hidden value="{{$id_evento}}" />
  <input name="id_sim_antigua" hidden value="{{$id_sim_antigua}}" />
...
<div class="row">
  <div class="col-md-3">
    ...
    <select class="form-control" name="cliente" id="cliente">
      ...
      @if($cli->id == $evento[0]->id_cliente)
      <option selected value="{{$cli->id}}">{{$cli->nombre}} {{$cli->apellido}}</option>
      @else
  ...
  <div class="col-md-4">
    ...
    <input value="{{$evento[0]->fecha_hora}}" type="datetime-local" class="form-control" id="fecha_hora" onclick="datetimepick()" name="fecha_hora" id="fecha_hora" required/>
  </div>
  <div class="col-md-3">
    <label>Dirección</label>
    <input class="form-control" value="{{$evento[0]->direccion}}" name="direccion" id="direccion" type="text" required />
  </div>
  <div class="col-md-1">
    <label>Buscar</label>
            ...
            <td>
              ...
              <select class="form-control" name="trabajadores[]">
                ...
                @foreach($trabajadores_lista as $trl)
                  @if($trl->id == $trabajadores_antiguos[$i]->id)
                    @if($trl->maneja)
                    <option selected value="{{$trl->id}}">{{$trl->nombre}} {{$trl->apellido}} (Sí maneja)</option>
                    @else
                    <option selected value="{{$trl->id}}">{{$trl->nombre}} {{$trl->apellido}} (No maneja)</option>
                    @endif
                  @else
                    @if($trl->maneja)
